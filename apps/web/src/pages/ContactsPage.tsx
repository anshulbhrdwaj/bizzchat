    ...
    <div className="flex-1 flex flex-col bg-[#F0F2F5] overflow-hidden">
      ...
      <header className="shrink-0 safe-area-top bg-[#075E54] text-white shadow-sm">
        <div className="flex items-center justify-between px-4 py-4">
          <h1 className="text-[20px] font-bold tracking-tight">Contacts</h1>
          <button
        ...
        <div className="px-3 pb-3">
          <input
            ...
            className="w-full h-9 px-4 rounded-xl bg-white/15 text-[14px] text-white placeholder:text-white/50 outline-none"
          />
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto pb-20 md:pb-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[17px] font-semibold text-gray-800">{search ? 'No contacts found' : 'No contacts yet'}</p>
            <p className="text-[14px] text-gray-400 mt-1">{search ? 'Try a different search' : 'Add contacts to get started'}</p>
          </div>
        ) : (
          <div className="mx-4 mt-4 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            {filtered.map((contact: any, idx: number) => {
              ...
              return (
                <div
                  key={contact.id}
                  className={`flex items-center gap-3 px-4 py-3 active:bg-gray-50 ${idx < filtered.length - 1 ? 'border-b border-gray-50' : ''}`}
                >
                  {/* Avatar */}
                  <div className="relative shrink-0">
                    <div className="w-[50px] h-[50px] rounded-full flex items-center justify-center overflow-hidden bg-[#128C7E]/10">
                      {contact.avatarUrl ? (
                        <img src={contact.avatarUrl} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-[16px] font-bold text-[#128C7E]">{initials}</span>
                      )}
                    </div>
                    ...
                    {editingId === contact.id ? (
                      <div className="flex gap-2">
                        <input autoFocus type="text" value={editName} onChange={e => setEditName(e.target.value)}
                          className="flex-1 h-9 px-3 rounded-xl bg-gray-100 text-[15px] text-gray-900 outline-none border border-gray-200 focus:border-[#128C7E]" />
                        <button onClick={() => updateContact.mutate({ id: contact.id, savedName: editName })}
                          className="px-4 h-9 rounded-xl bg-[#128C7E] text-white text-[13px] font-semibold">Save</button>
                      </div>
                    ) : (
                      <>
                        <p className="text-[16px] font-medium text-gray-900 truncate">{contact.displayName}</p>
                        <p className="text-[13px] text-gray-400 truncate">{contact.phone}</p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    }} className="w-9 h-9 flex items-center justify-center text-gray-300 rounded-xl active:bg-gray-100 transition-colors">
                    ...
                    <button onClick={() => { if (confirm('Remove this contact?')) deleteContact.mutate(contact.id) }}
                      className="w-9 h-9 flex items-center justify-center text-red-400 rounded-xl active:bg-red-50 transition-colors">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" strokeWidth="2">
                    ...
                    <button onClick={() => startChat(contact.contactId)}
                      className="w-9 h-9 flex items-center justify-center text-[#128C7E] rounded-xl active:bg-[#128C7E]/10 transition-colors">
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 backdrop-blur-sm animate-fade-in"
          ...
          <div className="w-full max-w-lg bg-white rounded-t-[32px] p-6 animate-slide-up shadow-2xl">
            <div className="w-10 h-1 bg-gray-200 rounded-full mx-auto mb-5" />
            <h2 className="text-[22px] font-bold text-gray-900 mb-1">New Contact</h2>
            ...
            <form onSubmit={e => { e.preventDefault(); addContact.mutate({ phone: newPhone, savedName: newName }) }}>
              <div className="space-y-3 mb-5">
                <input type="text" placeholder="Name (optional)" value={newName} onChange={e => setNewName(e.target.value)}
                  className="w-full h-12 px-4 rounded-2xl bg-gray-50 text-[16px] text-gray-900 outline-none border border-gray-200 focus:border-[#128C7E] transition-colors" />
                <input type="tel" placeholder="+1234567890" value={newPhone}
                  onChange={e => { setNewPhone(e.target.value); setAddError('') }}
                  className="w-full h-12 px-4 rounded-2xl bg-gray-50 text-[16px] font-medium text-gray-900 outline-none border border-gray-200 focus:border-[#128C7E] transition-colors" />
              </div>
              ...
              <button type="submit" disabled={addContact.isPending || newPhone.length < 5}
                className="w-full py-4 rounded-2xl bg-[#128C7E] text-white text-[16px] font-bold disabled:opacity-50 active:bg-[#075E54] transition-colors shadow-lg shadow-[#128C7E]/20">
