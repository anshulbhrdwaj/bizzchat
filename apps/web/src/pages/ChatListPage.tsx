  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col bg-white">
        <ChatHeader showSearch={showSearch} search={search} onSearch={setSearch} onToggleSearch={setShowSearch} />
    ...
    return (
      <div className="flex-1 flex flex-col bg-white">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-8">
          ...
          <button onClick={() => refetch()} className="px-6 py-2.5 rounded bg-[#128C7E] text-white text-[14px] font-medium">Retry</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex-1 flex flex-col bg-white">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 px-8">
          ...
          <p className="text-[15px] text-gray-600 text-center">No conversations yet</p>
          <p className="text-[13px] text-gray-400 text-center">Start chatting with businesses or contacts</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex-1 overflow-y-auto pb-20 md:pb-4">
            ...
            >
              ...
              <div className="flex-1 min-w-0 py-3 pr-4 border-b border-gray-100 flex flex-col justify-center">
                <div className="flex items-center justify-between gap-2 mb-0.5">
                  ...
                  <span className={`text-[12px] shrink-0 ${unread > 0 ? 'text-[#25D366] font-medium' : 'text-gray-500'}`}>
        ...
        {filtered.length === 0 && search && (
          <div className="flex flex-col items-center py-16">
            <p className="text-[14px] text-gray-500">No chats matching "{search}"</p>
          </div>
        )}
      </div>
...
/* ─── WhatsApp Header ─────────────────────────────────── */
function ChatHeader({ showSearch, search, onSearch, onToggleSearch }: {
  showSearch: boolean; search: string; onSearch: (v: string) => void; onToggleSearch: (v: boolean) => void;
}) {
  return (
    <header className="shrink-0 safe-area-top">
      {showSearch ? (
        <div className="flex items-center gap-2 px-2 py-2 bg-white border-b border-gray-200">
          <button onClick={() => { onSearch(''); onToggleSearch(false) }}
          ...
          <input type="text" placeholder="Search..." value={search} onChange={e => onSearch(e.target.value)} autoFocus
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[#075E54] text-white">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => onToggleSearch(true)}
